...
class Parser(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        return render(request, "tiempo/synop.html", context={'segment': 'suynop_parser'})
        ...
        try:
            ...
            for element in elements:  
                if element.lower().count("aaxx") > 1:
                    context['msg'] = {'type': 'error', 'title': 'Error',
                ...
                for val in synop:
                    ...
                    if etapa == 0:
                        if pos == 1:
                            obj.tipo_synop = CODIGO_TIPO_ESTACION[str(
                                value).upper()]
                            warning = True

                        elif pos == 2:
                            obj.fecha = datetime.datetime(
                                currentDate.year, currentDate.month, int(value[0:2]), int(value[2:4]), 0, 0)
                    ...
                    if etapa == 1:
                        ...
                        if value[0] == '1' and pos > 5:
                            if  value[2:5] != '///':
                                obj.vat_TTT = float(value[2:5])/10 if value[1] == '0' else float(value[2:5])/10 * -1
                            warning = True
                            ...
                            if value[1] == '9':
                                ...
                                if value[2:5] != '///':
                                    obj.var_TTT_rocio = float(
                                        value[2:5])/10 if value[1] == '0' else float(value[2:5])/10 * -1
                        ...
                        if value[0] == '0':
                            ...
                            if value[2:5]!='///':
                                obj.var_TTT_maxima = float(
                                    value[2:5])/10 if value[1] == '0' else float(value[2:5])/10 * -1
                            warning = True
                        elif value[0] == '2':
                            if value[2:5]!='///':
                                obj.var_TTT_minima = float(
                                    value[2:5])/10 if value[1] == '0' else float(value[2:5])/10 * -1
                            warning = True
                        elif value[0] == '3':
                            ...
                            warning = True
                            if value[2:5] != '///' :
                                obj.var_TT_suelo = float(
                                    value[3:5]) if value[2] == '0' else float(value[3:5]) * -1
                            ...
                            if value[1] == '4':
                                ...
                                obj.var_dT = cambio_temperatura[value[4]] if value[3] == '0' else f'-{cambio_temperatura[value[4]]}'
                        ...
                        if value[0] == '1':
                            ...
                            if value[1:5]!='////':
                                if value[1] != '4' and value[1] != '7':
                                    aux = value
                                    obj.var_dd_ff = f'{direccion[value[1:3]]} - {value[3:5]}kn'
                                elif value[1] == '4':
                                    obj.var_HHH_rafaga = f'{value[2:4]}:{int(value[4])*6}'
                                elif value[1] == '7':
                                    obj.var_dd_ff = f'{direccion[aux[1:3]]} - {value[2:5]}kn'
                        elif value[0] == '8':
                            warning = True
                            if value[1:5] != '////':
                    ...
                    if warning == False:
                        warnings.append(f'Código {codigo} - sección {etapa} - valor {value}')
...
class SynopListView(LoginRequiredMixin,View):
    def get(self, request):
        ...
        context['synops'] = Synop.objects.all().order_by('-id').select_related('estacion')
    ...
    def post(self, request):
        ...
        context['synops'] = Synop.objects.filter(estacion=station ,fecha__gte=fecha_inicio, fecha__lte=fecha_fin)
        context['estaciones'] = Estacion.objects.all()
        return render(request, "tiempo/synop_list.html", context=context)
...
class DatosAdinocialesListView(LoginRequiredMixin,View):
    def get(self, request, *args, **kwargs):
        ...
        context['datos_adinociales'] = Datos_adinociales.objects.filter(synop = code).values()
        context['code'] =  code
...
class PronosticoListView(LoginRequiredMixin,View):
    def get(self, request):
        ...
        context['pronosticos'] = Pronostico.objects.all().order_by('-id').select_related('provincia')
...
class Resumen(LoginRequiredMixin,View):
    ...
    pronostico_general = []
    ESTACION =  ''
    colores= ['#F66868','#1020F5','#68C7F6','#68F68E','#E8F668','#0D0D0C','#D610F5',]
    datos_estaciones = {}
    ...
    def init(self):
        ...
        synops = Synop.objects.filter(fecha__gte=f'{self.year}-{self.month}-{1}', fecha__lte=f'{self.year}-{self.month}-{self.day[1]}').order_by('-fecha', '-hora').select_related('estacion')
        self.pronostico_general = []
        for element in self.datos_estaciones:
            self.datos_estaciones[element]['data'] = [0 for _ in range(1, self.day[1]+1)]
            self.datos_estaciones[element]['temp_date'] = [[] for _ in range(1,self.day[1]+1)]
        ...
        for synop in synops:
            ...
            if synop.vat_TTT != -999:
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.vat_TTT)
            if self.datos_estaciones[synop.estacion.name]['last_synops_day']['day'] == '':
                self.datos_estaciones[synop.estacion.name]['last_synops_day']['day'] = synop_date
                self.datos_estaciones[synop.estacion.name]['last_synops_day']['synops'].append(synop)
            elif self.datos_estaciones[synop.estacion.name]['last_synops_day']['day'] == synop_date:
                self.datos_estaciones[synop.estacion.name]['last_synops_day']['synops'].append(synop)
        ...
        for elemen in self.datos_estaciones:
                ...
                for synop in self.datos_estaciones[elemen]['last_synops_day']['synops'] :
                    ...
                    if synop.var_RRR_presipitacion !=-999 and synop.var_RRR_presipitacion !=990:
                ...
                valores['vat_TTT'] = round(statistics.mean(temp),2) if len(temp) > 0 else  '--'
                valores['var_TTT_maxima'] = mayor(temp_max) if len(temp_max) > 0 else  '--'
                valores['var_TTT_minima'] = menor(temp_min) if len(temp_min) > 0 else  '--'
                valores['var_TTT_rocio'] = round(statistics.mean(temp_rocio),2) if len(temp_rocio) > 0 else  '--'
                valores['var_TT_suelo'] =round(statistics.mean(min_12),2) if len(min_12) > 0 else  '--'
                valores['var_UUU'] = round(statistics.mean(humedad),2) if len(humedad) > 0 else  '--'
                
                valores['var_ff'] = round(statistics.mean(viento),2) if len(viento) > 0 else  '--'
                ...
                valores['var_ff_ms'] = round(statistics.mean(viento)* 0.514,2) if len(viento) > 0 else  '--'
                valores['var_ff_km'] = round(statistics.mean(viento)* 1.85,2) if len(viento) > 0 else  '--'
                valores['var_RRR'] = round(statistics.mean(precipitacion),2) if len(precipitacion) > 0 else  '--'
                valores['var_RRR_presipitacion'] = round(mayor(acumulada),2) if len(acumulada) > 0 else  '--'
                valores['var_RRR_in'] = round((statistics.mean(precipitacion)*0.039),2) if len(precipitacion) > 0 else  '--'
                valores['var_RRRR'] = round(statistics.mean(ante_24),2) if len(ante_24) > 0 else  '--'
                valores['var_PPPP'] = round(statistics.mean(presion),2) if len(presion) > 0 else  '--'
                valores['var_PPPP_mar'] = round(statistics.mean(mar),2) if len(mar) > 0 else  '--'
                valores['var_PPP_presion'] = round(statistics.mean(variasion),2) if len(variasion) > 0 else  '--'
    ...
    def get(self, request):
        ...
        for value in self.datos_estaciones:
            for i in range(len(self.datos_estaciones[value]['data'])):
                ...
                self.datos_estaciones[value]['data'][i] = round(statistics.mean(list),2) if len(list) > 0 else  0 
            elements.append({
                'label': value,
    ...
    def post(self, request):
        ...
        titulos = {'temp': 'Temperatura','temp_min': 'Temperatura minima','temp_max': 'Temperatura maxima','temp_rocio': 'Temperatura rocio','humedad': 'Humedad','presion': 'Presión','viento': 'Velocidad Viento','precip': 'Precipitaciones', }
        ...
        synops = Synop.objects.filter(
            fecha__gte=fecha_inicio, fecha__lte=fecha_fin).select_related('estacion')
        year, month, day = (int(x) for x in fecha_inicio.split('-'))
        year_fin, month_fin, day_fin = (int(x) for x in fecha_fin.split('-'))

        for element in self.datos_estaciones:
            self.datos_estaciones[element]['data'] = [0 for _ in range(day, day_fin+1)]
            self.datos_estaciones[element]['temp_date'] = [[] for _ in range(day, day_fin +1)]
        ...
        for synop in synops:
            ...
            if variable == 'temp' and synop.vat_TTT != -999.0:
                ...
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.var_TTT_minima)
            elif variable == 'temp_max' and synop.var_TTT_maxima != -999:
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.var_TTT_maxima)
            elif variable == 'temp_rocio' and synop.var_TTT_rocio != -999:
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.var_TTT_rocio)
            elif variable == 'humedad' and synop.var_UUU != -999:
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.var_UUU)
            elif variable == 'presion' and synop.var_PPPP != -999:
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.var_PPPP)
            elif variable == 'viento' and synop.var_ff != 'No proporcionado':
                if 'm/s' in synop.indicador_viento_Iw :
                    self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(round((float(synop.var_ff) * 1.943),2))
                else:
                    self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(float(synop.var_ff))
            elif variable == 'precip' and synop.var_RRR != -999 and synop.var_RRR != 990:
                self.datos_estaciones[synop.estacion.name]['temp_date'][synop_date].append(synop.var_RRR )
...
class Utiles(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        self.init()
        synops = Synop.objects.filter(fecha=fecha).order_by('hora').select_related('estacion')
        xvalues = []
        for synop in synops:
        ...
        for element in self.datos_estaciones:
            for i in range(0,len(xvalues)):
                ...
                for synop in synops:
                    if xvalues[i] == synop.hora and element['code'] == synop.estacion.code and synop.vat_TTT != -999 :
    ...
    def post(self,request):
        ...
        variable = request.POST.get('variable')
        titulos = {'temp': 'Temperatura','temp_min': 'Temperatura mínima','temp_max': 'Temperatura máxima','temp_rocio': 'Temperatura punto de rocío','humedad': 'Humedad','presion': 'Presión','viento': 'Velocidad Viento en kn','precip': 'Precipitaciones', }
        ...
        for element in self.datos_estaciones:
            for i in range(0,len(xvalues)):
                element['data'].append(0) 
                for synop in synops:
                    if xvalues[i] == synop.hora and element['code'] == synop.estacion.code:
                        ...
                        if variable == 'temp' and synop.vat_TTT != -999:
                            ...
                            if 'm/s' in synop.indicador_viento_Iw :
                                 element['data'][i] = round((float(synop.var_ff) * 1.943), 2)
                            else:
