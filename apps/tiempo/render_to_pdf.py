...
class GeneralViewPDF(View):
    def get(self, request, *args, **kwargs):
        ...
        synops_day = Synop.objects.filter(var_IIiii_code=estacion, fecha=fecha).values()
        (
            temp,
            temp_min,
            temp_max,
        ...
        object["var_TTT_rocio"] = (
            statistics.mean(temp_rocio) if len(temp_rocio) > 0 else "--"
        )
        ...
        object["var_dd"] = (
            round(statistics.mean(v_direccion), 2) if len(v_direccion) > 0 else "--"
        )
        object["var_direccion"] = (
            calc_dir_vient(statistics.mean(v_direccion))
            if len(v_direccion) > 0
            else "--"
        )
        object["var_ff_ms"] = (
        ...
        object["var_RRR"] = (
            statistics.mean(precipitacion) if len(precipitacion) > 0 else "--"
        )
        object["var_RRR_presipitacion"] = (
            statistics.mean(acumulada) if len(acumulada) > 0 else "--"
        )
        object["var_RRR_in"] = (
        ...
        object["var_PPP_presion"] = (
            statistics.mean(variasion) if len(variasion) > 0 else "--"
        )
...
class ReporteDiario(View):
    ...
    def post(self, request):
        ...
        synops_day = Synop.objects.filter(estacion=station, fecha=fecha).select_related(
            "estacion"
        )
        (
            temp,
        ...
        object["var_ff"] = (
            round(statistics.mean(viento), 1) if len(viento) > 0 else "--"
        )
        object["viento_km"] = (
            round((statistics.mean(viento) * 1.85), 1) if len(viento) > 0 else "--"
        )
        object["viento_m"] = (
            round((statistics.mean(viento) * 0.514), 1) if len(viento) > 0 else "--"
        )
        ...
        object["var_RRR"] = (
            round(statistics.mean(precipitacion), 2) if len(precipitacion) > 0 else "--"
        )
        object["var_RRR_presipitacion"] = (
            round(statistics.mean(acumulada), 2) if len(acumulada) > 0 else "--"
        )
        object["var_RRR_in"] = (
            round((statistics.mean(precipitacion) * 0.039), 2)
            if len(precipitacion) > 0
